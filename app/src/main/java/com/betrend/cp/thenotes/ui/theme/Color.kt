package com.betrend.cp.thenotes.ui.theme

import androidx.compose.ui.graphics.Color

val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)
val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)

val White = Color(0xFFFFFFFF)
val Graffit = Color(0xFF404040)
val GraffitL = Color(0xFF808080)
val GraffitLL = Color(0xFFA2A2A2)
val GraffitLLL = Color(0xFFBDBDBD)
val GraffitD = Color(0xFF303030)
val GraffitDD = Color(0xFF202020)
val Transparent = Color(0x00000000)

val YellowNote = Color(0xFFFFF591)
val YellowNoteD = Color(0xFFFFF061)
val YellowNoteDD = Color(0xFFECD822)
val YellowNoteDDD = Color(0xFFA79D3C)
val YellowNoteL = Color(0xFFFDFBA3)
val YellowNoteLL = Color(0xFFFFFAC5)
val YellowNoteLLL = Color(0xFFFFFFDA)
val YellowNote4 = Color(0xFFFFFEF4)
val YellowNote5 = Color(0xFFFFFEF1)

val GreenNote  = Color(0xFF8CFF91)
val GreenNoteL = Color(0xFFD5FFD6)
val GreenNoteD = Color(0xFF63FF68)
val GreenNoteDD = Color(0xFF00E207)

val NoteError = Color(0xFFFFA295)
val NoteItemError = Color(0xFFFF6E5A)

val ButtonL = Color(0xFFFFEB3B)
val ButtonD = Color(0xFFD9D55B)
val BorderBtn = Color(0xFFFFF59D)

val BackTxtD = Color(0xFFFFF4C3)